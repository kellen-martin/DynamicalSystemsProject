...
def main():
    ...
    r_0 = np.array([[0.0, 0.0, 0.0], [147.1*10**9, 0, 0], [147.1*10**9+Earth_SOI, 0, 0]])   # [m]
    v_0_1 = np.array([[0.0, 0.0, 0.0], [0, 29.783*10**3, 0], [deltav_j_1, 29.783*10**3 + deltav_i_1, 0]])        # [m/s]
    v_0_2 = np.array([[0.0, 0.0, 0.0], [0, 29.783*10**3, 0], [deltav_j_2, 29.783*10**3 + deltav_i_2, 0]])        # [m/s]
    v_0_3 = np.array([[0.0, 0.0, 0.0], [0, 29.783*10**3, 0], [deltav_j_3, 29.783*10**3 + deltav_i_3, 0]])        # [m/s]
    ...
    plt.plot(delta_r_1[:,1],delta_r_1[:,0], label = "2000 m/s")
    plt.plot(delta_r_2[:,1],delta_r_2[:,0], label = "5000 m/s")
    plt.plot(delta_r_3[:,1],delta_r_3[:,0], label = "10000 m/s")
    plt.plot(a,b)
    plt.xlabel("x [m]")
    plt.ylabel("y [m]")
    plt.legend()
    plt.title("1-year Earth-Earth Transfer")
    plt.show()
    # Plots
